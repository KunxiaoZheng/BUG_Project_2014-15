...
unsigned int MAX_TCP_SOCKETS = TCP_SOCKET_COUNT;
...
// Added by Keith Vogel, this will include the FIN byte that was taken out above
MyTCB.MySEQ++;      
						SendTCP(ACK, 0);
						CloseSocket();
						return;
...
/****************************************************************************
  Section:
	Added for chipKIT functionality
  ***************************************************************************/

/*****************************************************************************
  Function:
	void GetSocketEndPoints(TCP_SOCKET hTCP, IP_ADDR * pRemoteIP, MAC_ADDR * pRemoteMAC, WORD * pdwRemotePort, WORD * pdwLocalPort)
  ...
  Parameters:
    ...
    pdwRemotePort - pointer to a WORD to receive the remote port connected to.
    pdwLocalPort - pointer to a WORD to receive the local port in this connection
 ...
 ***************************************************************************/
void GetTcpSocketEndPoints(TCP_SOCKET hTCP, IP_ADDR * pRemoteIP, MAC_ADDR * pRemoteMAC, WORD * pdwRemotePort, WORD * pdwLocalPort)
{
	SyncTCBStub(hTCP);
	SyncTCB();
    ...
    *pRemoteIP = MyTCB.remote.niRemoteMACIP.IPAddr;
    *pRemoteMAC = MyTCB.remote.niRemoteMACIP.MACAddr;
    *pdwRemotePort = MyTCB.remotePort.Val;
    *pdwLocalPort = MyTCB.localPort.Val;
}

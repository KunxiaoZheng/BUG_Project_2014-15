...
BOOL             g_MgmtReadMsgReady;                  /* TRUE if rx mgmt msg to process, else FALSE              */
volatile BOOL    g_ExIntNeedsServicing;               /* TRUE if external interrupt needs processing, else FALSE */
...
void WiFiTask(void)
{
    ...
    if ((!g_WaitingForMgmtResponse) && (GetAppPowerSaveMode() == TRUE))
    {
        ...
        else if (DHCPIsEnabled(0) && (g_DhcpSuccessful == TRUE))
        {
...
static void ProcessInterruptServiceResult(void)
{
    ...
    if((hostInt & WF_HOST_INT_MASK_INT2) == WF_HOST_INT_MASK_INT2)
    {
        ...
        if (hostInt2 & WF_HOST_INT_MASK_MAIL_BOX_0_WRT)
        {
            ...
            #if defined(MRF24WG) && !defined(__Digilent_Build__)
                  WF_DisplayModuleAssertInfo();
 
           #endif

#if 0
// temp code added to get the assert info
// I == lower 24 bits
// M == upper 8 bits
{   
volatile UINT32 assertInfo;

...
/*****************************************************************************
  ...
  Remarks:
 ***************************************************************************/
void DWIFIcK_InitIntVector(void)
{
    // if we are running a version of MPIDE with installable vector handler
    // then we need to install our vector handler
    // we do not need to remove it as we never really completely shutdown
    // the DNETcK engine
#if defined(_IMAGE_EBASE_ADDR)
    setIntVector(MRF24_INT_VEC, _WFInterrupt);
    setIntPriority(MRF24_INT_VEC, 3, 0);
#endif
}

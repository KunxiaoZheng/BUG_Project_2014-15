...
********************************************************************
 ...
 chipKIT  KeithV/Digilent - converted the file to a C++ file so that it
            may be referenced as a class from Arduino sketches.
********************************************************************/
...
// this is because Arduino defines BYTE as 0, and we don't want that.
#undef BYTE
#define BYTE uint8_t
...
// Write functions
    ...
    CETYPE PopulateEntries(FILEOBJ fo, char *name , WORD *fHandle, BYTE mode);
...
FSFILE * FSfopen( const char * fileName, const char *mode )
{
    ...
    if(FILEfind (filePtr, &gFileTemp, LOOK_FOR_MATCHING_ENTRY, 0) == CE_GOOD)
    {
        ...
        switch(ModeC)
        {
            ...
            default:
                FSerrno = CE_INVALID_ARGUMENT;
                final = (CETYPE) 0xFF;;  //indicate error condition
                break;
        }
    }
    else
...
CETYPE FILECreateHeadCluster( FILEOBJ fo, DWORD *cluster)
{
    ...
    *cluster = FATfindEmptyCluster(fo);
    ...
    else
    {
        ...
        if(error == CE_GOOD)
        {
            error = (CETYPE) EraseCluster(disk,*cluster);
        }
    }
    ...
    while (error == CE_GOOD && count > 0)
    {
        ...
        if (pos == dsk->sectorSize)
        {
            ...
            if (stream->sec == dsk->SecPerClus)
            {
                ...
                {
                    error = (CETYPE) FILEallocate_new_cluster(stream, 0);    // add new cluster to the file
                    needRead = FALSE;
                }
                else
                    error = (CETYPE) FILEget_next_cluster( stream, 1);
            }
...
size_t FSfread (void *ptr, size_t size, size_t n, FSFILE *stream)
{
    ...
    while( len )
    {
        ...
        if( pos == dsk->sectorSize )
        {
            ...
            {
                stream->sec = 0;
                if( (error = (CETYPE) FILEget_next_cluster( stream, 1)) != CE_GOOD )
                {
